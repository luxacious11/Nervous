...
/***** Species Stats *****/
function formatSpeciesStats(data) {
	data.sort((a, b) => {
	...
	let html = `<div class="h6">Last updated: ${data[0].Timestamp}</div>`;
	...
	data.forEach(species => {
		if(species.Breakdown && species.Breakdown !== '') {
			html += `<tr class="overall">
				<td>
				    <button onClick="expandBreakdown(this)"
				      data-species="${cleanText(species.Species)}">
					<i class="fa-solid fa-plus"></i>
					<i class="fa-solid fa-minus"></i>
				        ${capitalize(species.Species, [` `, `-`])}
				    </button>
				</td>
				<td>${species.Count}</td>
			</tr>`;

			let breakdown = JSON.parse(species.Breakdown);
			breakdown.sort((a, b) => {
				if(a.form < b.form) {
					return -1;
			...
			breakdown.forEach((section, i) => {
				let extraClass = ``;
				if (i === 0) {
					extraClass = `no-top-border`;
				} else if (i === breakdown.length - 1) {
					extraClass = `add-bottom-border`;
				}
				html += `<tr class="breakdown ${extraClass}" data-species="${cleanText(species.Species)}">
					<td>${capitalize(section.form, [` `, `-`])}</td>
					<td>${section.count}</td>
				</tr>`;
			});
		} else {
			html += `<tr class="overall">
				<td>${capitalize(species.Species, [` `, `-`])}</td>
				<td>${species.Count}</td>
			</tr>`;
		}
	});
	...
	document.querySelector(`tag-tab[data-key="#stats"] .webpage--content-inner`).innerHTML = html;
}
function expandBreakdown(e) {
	const species = e.dataset.species;
	e.classList.toggle('is-open');
	document.querySelectorAll(`.breakdown[data-species="${species}"]`).forEach(row => row.classList.toggle('is-open'));
}
...
function formatSpecies(data) {
    ...
    data.forEach(species => {
        html += formatSpeciesBlock(species);
    });

    document.querySelector('#webpage--rows').insertAdjacentHTML('beforeend', html);
}

function formatSpeciesBlock(species) {
    let creditsArray = JSON.parse(species.Credit).map(item => `<a href="?showuser=${item.userid}">${item.username}</a>`);
    let credits = ``;
    ...
    let abilities = JSON.parse(species.Strengths).map(item => `<li>${item}</li>`).join('');
    let weaknesses = JSON.parse(species.Weaknesses).map(item => `<li>${item}</li>`).join('');

    return `<div class="species--item grid-item ${species.Tags} aware-${species.Awareness} know-${species.Knowledge}">
        <div class="species">
            <div class="species--main">
                <div class="species--title">${capitalize(species.Species, [' ', '-'])}</div>
                <div class="species--type">${species.Premium === 'TRUE' ? 'Premium' : 'Free'}</div>
            </div>
            <div class="species--stats">
                <div class="species--stat"><b>aging</b> ${species.Aging}</div>
                <div class="species--stat"><b>lifespan</b> ${species.Lifespan}</div>
                <div class="species--stat"><b>created by</b> ${credits}</div>
            </div>
            <div class="species--overview species-accordion">
                <div class="accordion--trigger">
                    <b>Physiology</b>
                    <i class="fa-solid fa-xmark close-symbol"></i>
                    <i class="fa-solid fa-plus open-symbol"></i>
                </div>
                <div class="accordion--content"><div class="scroll">${species.Physiology}</div></div>
            </div>
            <div class="species--overview species-accordion">
                <div class="accordion--trigger">
                    <b>Community</b>
                ...
                <div class="accordion--content"><div class="scroll">${species.CommunityStructure}</div></div>
            </div>
            <div class="species--overview species-accordion">
                <div class="accordion--trigger">
                    <b>Abilities</b>
                ...
                <div class="accordion--content"><div class="scroll"><ul>${abilities}</ul></div></div>
            </div>
            <div class="species--overview species-accordion">
                <div class="accordion--trigger">
                    <b>Weaknesses</b>
                ...
                <div class="accordion--content"><div class="scroll"><ul>${weaknesses}</ul></div></div>
            </div>
        </div>
    </div>`;
}